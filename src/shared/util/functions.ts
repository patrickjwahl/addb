...
export const partitionsOrder = divisionsOrder.map(key => `_${key}`).concat(Object.keys(friendlyGPA).sort().map(key => `_${key}`)).concat(divisionsOrder.flatMap(div => Object.keys(friendlyGPA).sort().map(gpa => `_${div}_${gpa}`)))
