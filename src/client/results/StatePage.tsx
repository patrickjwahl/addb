import API from '@/client/API'
import StatePageYearDisplay from '@/client/results/StatePageYearDisplay'
import { StatePage as StatePageType } from '@/shared/types/response'
import { taglines } from '@/shared/util/consts'
import { groupBy } from '@/shared/util/functions'
import { useEffect, useState } from 'react'
import { Helmet } from 'react-helmet'
import { ColorRing } from 'react-loader-spinner'
import { useParams } from 'react-router-dom'

export default function StatePage() {

    const [data, setData] = useState<StatePageType | null>(null)

    const params = useParams()

    const performSearch = async () => {
        const results = (await API.getStateResults(params.name || '')).data
        if (results) {
            setData(results)
        }
    }

    useEffect(() => {
        performSearch()
    }, [params.name])

    const stateName: string = params.name || ''

    if (!data) {
        return <ColorRing
            height={40}
            width={40}
            wrapperStyle={{ marginTop: '50px' }}
            visible
        />
    }

    let retval

    let matchesByYear = groupBy(data.matches || [], match => match.year)
    let nationalsByYear = groupBy(data.nationals || [], performance => performance.match.year)

    let resultsFound = ((data.matches?.length || 0) > 0 || data.nationals.length > 0)

    let yearSet = new Set(Object.keys(matchesByYear))
    yearSet = yearSet.union(new Set(Object.keys(nationalsByYear)))

    const years = Array.from(yearSet).map(Number).sort((a, b) => b - a)
    let yearViews = []
    for (const year of years) {
        yearViews.push(<StatePageYearDisplay matches={matchesByYear[year] || []} nationalsPerformances={nationalsByYear[year] || []} year={year} stateName={stateName} />)
    }

    const { name } = params

    if (resultsFound && name) {
        retval = (
            <div className='state-page-container'>
                <Helmet><title>{name.replace('_', ' ')} | AcDecDB</title></Helmet>
                <div className={'state-page-header'} style={{ backgroundImage: `url(/img/${name}.jpg)` }}>
                    <div className='state-page-header-text'>
                        {/* <img src={`/img/${name}.jpg`} height={50} /> */}
                        <div className='state-tagline' style={{ display: (name in taglines) ? 'initial' : 'none' }}>"{taglines[name]}"</div>
                        <h2>{name.replace('_', ' ').toUpperCase()}</h2>
                    </div>
                </div>
                {yearViews}
            </div>
        )
    } else {
        if (!name) retval = (null); else {
            retval = (
                <div className='state-page-container'>
                    <Helmet><title>{name.replace('_', ' ')} | AcDecDB</title></Helmet>
                    <div className={'state-page-header'} style={{ backgroundImage: `url(/img/${name}.jpg)` }}>
                        <div className='state-page-header-text'>
                            {/* <img src={`/img/${name}.jpg`} height={50} /> */}
                            <div className='state-tagline' style={{ display: (name in taglines) ? 'initial' : 'none' }}>"{taglines[name]}"</div>
                            <h2>{name.replace('_', ' ').toUpperCase()}</h2>
                        </div>
                    </div>
                    <div className='search-result-none'>No results found</div>
                </div>
            )
        }
    }

    return retval
}
